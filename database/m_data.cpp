...
m_data::m_data() {
    this->count = 0;
    this->child = unordered_map<int64_t, unordered_set<int64_t>>();
    this->free = vector<int64_t>();
    this->pos = unordered_map<string, int64_t>();
    this->path = unordered_map<int64_t, string>();
    child[0] = unordered_set<int64_t>();
    path[0] = "/lib";
    pos["/lib"] = 0;
}
